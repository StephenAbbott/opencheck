...
| `mca_india` | Ministry of Corporate Affairs — Company Master Data (India) | GODL-India | `in_cin` from GLEIF (`RA000394`) | India's national company register extract on the OGD Platform (data.gov.in) — CIN, name, status, class/category, authorised & paid-up capital, registration date, RoC, registered office address and NIC classification for ~3.67M companies. Entity statements only; no officer or ownership data; exact-match search (names uppercase). Non-EU (`NON_EU_JURISDICTION`). Requires a free `DATA_GOV_IN_API_KEY` |
