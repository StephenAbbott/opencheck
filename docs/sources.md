...
Forty-three active adapters, each implementing the same `SourceAdapter` protocol (`search`, `fetch`, `info`). Four further adapters are committed but inactive (bulk-data only) — see [Inactive / bulk-only adapters](#inactive--bulk-only-adapters) below.
...
| `onrc_romania` | ONRC — Oficiul Național al Registrului Comerțului (Romania) | CC-BY-4.0 | ONRC registration number (either format), from GLEIF `RA000497` | Not in `REGISTRY`, not wired. Companies and their **legal representatives** with the register's own role wording and their filed dates of birth, from the monthly data.gov.ro dump (six `^`-delimited CSVs, 1.6 GB; 4,219,081 companies and 3,689,931 representative rows on the 2 Sep 2026 export), built into a local SQLite index via `scripts/build_onrc_romania_index.py`. Sole-trader forms (PFA, II, PF, AF, IF — about 1.35M rows) are excluded as natural persons. No shareholders and no beneficial owners: both are behind ONRC's paid *certificat constatator*. **This index is also what makes `anaf_romania` reach more than half of Romanian LEI holders** — it maps a trade-register number to the fiscal code ANAF is keyed on, taking resolution from 47.8% to 98.1% (1,800 live records, 14 Sep 2026). Activate via `ONRC_ROMANIA_DB_FILE` |
