...
| `OPENCHECK_CH_GRAPH_FIRST` | Reserved for Phase 188: walk the corporate-PSC chain above a UK subject on the local graph instead of calling the register hop by hop. Default `false` until a live-versus-local comparison on the curated UK examples; today it only appears on `GET /pscgraph`. |
