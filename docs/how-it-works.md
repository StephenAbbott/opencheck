...
4. Dispatches to every other adapter using whichever identifier they understand:
   ...
   - **INPI (France)** — fetched by `fr_siren` (derived from GLEIF RA code `RA000189` or `RA000192`, spaces removed); a SIREN the RNE does not hold — usually an association or a foundation — shows a short "not in the register" note instead of a source error; delivers company profile and officers as BODS statements via the Registre National des Entreprises API. Individual persons in `composition.pouvoirs` with `typeDePersonne == "INDIVIDU"` and `beneficiaireEffectif == false` are mapped to person + relationship statements using the full 65-code `roleEntreprise` codelist; BO records (`beneficiaireEffectif == true`) are silently excluded per Loi Sapin II.
...
| `GET /export?lei=<LEI>&format=zip\|json\|jsonl\|xml\|csv\|xlsx\|cypher\|senzing\|ftm\|gql\|amlai\|rdf` | Downloadable BODS bundle. `zip` ships `bods.json` + `bods.jsonl` + `bods.xml` + `senzing.jsonl` + `ftm.jsonl` + `manifest.json` + `LICENSES.md`; `json` / `jsonl` / `xml` return the statements only; `csv` returns the entity / person / ownership-edge tables as a zip, `xlsx` the same three tables as one Excel workbook, and `cypher` a Neo4j Cypher script (the same writer `POST /export-network` uses). The `ftm` format projects the bundle into [FollowTheMoney entities](https://followthemoney.tech/) (newline-delimited `{id, schema, properties}`, ready for `alephclient write-entities`, the `ftm` CLI and OpenSanctions matching): companies and people become `Company` / `Person` / `PublicBody` nodes keyed by their BODS statementIds, and each disclosed interest becomes an `Ownership` (with `percentage` and direct/indirect `ownershipType`) or `Directorship` link — nodes always precede links so streaming loaders resolve references. The `xml` format uses the [canonical BODS v0.4 XML serialisation](https://github.com/StephenAbbott/bods-xml). The `senzing` format projects the BODS bundle into [Senzing JSON entity records](https://www.senzing.com/docs/entity_specification/) (newline-delimited, ready to load into Senzing for entity resolution): one record per entity/person with a `REL_ANCHOR`, and each disclosed ownership/control relationship folded into a `REL_POINTER` on the interested party. Each record also carries `DATA_LICENSE` and `ATTRIBUTION` payload attributes — the most-restrictive licence across that record's contributing sources, so a record combining a permissive and a non-commercial source is stamped non-commercial. Add `&subsidiaries=true` to opt the GLEIF subsidiary network into the bundle (off by default — a large group can add hundreds of statements; deduped by `statementId`). |
...
| `GET /history?lei=<LEI>&include_noise=<bool>` | The change history behind the History tab — every register that keeps a change log (GLEIF, Companies House, New Zealand, Estonia, Denmark) merged onto one shared model. See [docs/time-machine.md](time-machine.md). |
