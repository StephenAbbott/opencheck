...
**Latest: Phase 79** — Google AML AI export: feed an OpenCheck ownership graph into AML AI

`GET /export?format=amlai` packages the assembled BODS bundle as [Google AML AI](https://docs.cloud.google.com/financial-services/anti-money-laundering/docs/reference/schemas/aml-input-data-model) input tables, generated with [bods-aml-ai](https://github.com/StephenAbbott/bods-aml-ai) via its new in-memory API. AML AI has no party-to-party relationship table, so ownership is encoded through its extensibility mechanisms: numeric signals per party in `party_supplementary_data` (`bo_ownership_pct_<subject>`, BO flags, interest types) and synthetic "ownership accounts" in `account_party_link` linking owners to owned entities — the bundled README explains the encoding. The upstream change also fixed a latent bug: bods-aml-ai's output writer had been swallowed by an unanchored `output/` gitignore rule, breaking its pipeline on fresh clones. This completes the plan-§7.6 export roadmap — every promised format has shipped. Available on the Download data panel, the FullCheck network export, and `/export-network`.

*Previous: [Phase 78 — BigQuery GQL export: query an OpenCheck ownership graph with ISO GQL](docs/status.md)*
