...
`EXAMPLE_LEIS` (frontend/src/components/HomePanels.tsx) drives which curated
cards appear on the
...
# Phase 168 moved the homepage panels out of App.tsx; this guard followed
# them, and would have caught the move on its own if it had not.
EXAMPLES_TSX = REPO / "frontend" / "src" / "components" / "HomePanels.tsx"
...
def _frontend_leis() -> set[str]:
    text = EXAMPLES_TSX.read_text(encoding="utf-8")
    ...
    assert block, f"EXAMPLE_LEIS array not found in {EXAMPLES_TSX.name}"
