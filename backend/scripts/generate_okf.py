...
def _expected_auto_concepts() -> dict[Path, str]:
    """The auto-generated concepts (path -> content) derived from the registry."""
    exp: dict[Path, str] = {}
    for sid in sorted(REGISTRY):
        exp[OKF_ROOT / "sources" / f"{sid}.md"] = _source_concept(REGISTRY[sid])
    exp[OKF_ROOT / "sources" / "index.md"] = _sources_index()
    exp[OKF_ROOT / "licensing" / "matrix.md"] = _licensing_matrix()
    exp[OKF_ROOT / "licensing" / "index.md"] = _licensing_index()
    return exp
...
def generate() -> list[Path]:
    ...
    expected = _expected_auto_concepts()
...
def _norm(text: str) -> str:
    """Drop the timestamp line so a date-only difference isn't flagged as drift."""
    return "\n".join(
        ln for ln in text.splitlines() if not ln.strip().startswith("timestamp:")
    )


def stale_concepts() -> list[Path]:
    """Auto-generated concepts whose committed content differs from the registry."""
    stale: list[Path] = []
    for path, content in _expected_auto_concepts().items():
        current = path.read_text(encoding="utf-8") if path.exists() else ""
        if _norm(current) != _norm(content):
            stale.append(path)
    return stale
...
def main() -> int:
    ...
    ap.add_argument(
        "--check",
        action="store_true",
        help="validate OKF conformance and that generated concepts match the registry",
    )
    args = ap.parse_args()
    if args.check:
        rc = check()
        ...
        if stale:
            print(
                f"STALE: {len(stale)} generated OKF concept(s) differ from the live "
                "registry — run `python backend/scripts/generate_okf.py`:"
            )
            for p in stale:
                print("  ✗", p.relative_to(OKF_ROOT))
            rc = 1
        else:
            print("OKF generated concepts are in sync with the registry.")
        return rc
    written = generate()
