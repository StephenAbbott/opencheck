  ...
  useEffect(() => {
    ...
    const handle = requestAnimationFrame(() => {
      ...
      try {
        ...
        // Post-render fix for invisible edges from bods-dagre's Dr/Pr decorators.
        //
        // Dr/Pr pattern:
        //   1. Original dagre edge:  <g class="edgePath"> style="opacity: 0;"
        //   2. Styled clone:         <g class="edgePath own|control"> style="opacity: 1;"
        //
        // The clone's path is generated by BezierJS SVGtoBeziers().offset(n).
        // This fails in two ways depending on the edge geometry:
        //
        //   a) Collinear bezier (all control points share the same x, typical
        //      in a simple 2-node TB layout) → BezierJS produces NaN coordinates
        //      → clone path is "MNaN,NaN …" — invisible.
        //
        //   b) Nearly-straight bezier in a wide fan-out (e.g. GLEIF subsidiaries)
        //      → BezierJS .offset() returns an empty PolyBezier ({curves: []})
        //      → Br() tries to access curves[0][0].x → TypeError caught silently
        //      by d3's selection .each() → clone path attribute is never updated,
        //      left at the value dagre set before Dr/Pr ran (which may be an
        //      incomplete path stub like "M0,0") — also invisible.
        //
        // Two-pass fix:
        //   Pass 1 — restore ALL hidden original dagre edges unconditionally.
        //            This is the guaranteed visible fallback (dark grey lines,
        //            default dagre style) regardless of what happened to clones.
        //   Pass 2 — remove styled clones whose paths are invalid (NaN, empty,
        //            or a degenerate stub shorter than 10 chars). Valid clones
        //            remain and render on top of the restored originals.
        //
        // Net result:
        //   • Broken clone  → original (grey line) shows through.
        //   • Working clone → original also visible but clone's coloured,
        //                     offset path renders directly on top of it.

        // Pass 1: restore original edges.
        el.querySelectorAll<SVGGElement>("g.edgePath").forEach((edge) => {
          const isClone =
            edge.classList.contains("own") || edge.classList.contains("control");
          if (!isClone && edge.getAttribute("style")?.includes("opacity: 0")) {
            edge.style.opacity = "1";
            // Ensure the path has a visible stroke — dagre's default may be
            // absent or very light on some builds.
            const p = edge.querySelector<SVGPathElement>("path");
