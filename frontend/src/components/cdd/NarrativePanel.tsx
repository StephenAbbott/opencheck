...
/** Phase 217: the sign-off as a saved report froze it — read, never edited. */
function SavedDisposition({ value }: { value: DispState | undefined }) {
  if (!value) return null;
  const opt = DISP_OPTIONS.find((o) => o.status === value.status);
  return (
    <span className="mt-1 flex flex-wrap items-center gap-1.5">
      <span className={`rounded-full border px-2 py-0.5 text-oo-meta font-medium ${opt?.active ?? ""}`}>
        <span className="sr-only">Analyst decision: </span>
        {opt?.label ?? value.status}
      </span>
      {value.comment && <span className="text-oo-meta text-oo-muted">{value.comment}</span>}
    </span>
  );
}

function dispFromRecord(record: DispositionRecord | null): Record<string, DispState> {
  const out: Record<string, DispState> = {};
  for (const d of record?.dispositions ?? []) {
    out[d.claim_id] = { status: d.status, comment: d.comment ?? "" };
  }
  return out;
}
...
export function NarrativePanel({
  ...
  lei: string;
  /** Phase 217: on a saved report, the summary and sign-off exactly as they
   *  were saved. Nothing is generated, fetched or written: the panel reads
   *  what the record holds, and says so when it holds no summary. */
  saved?: ReportExportPayload | null;
  ...
  const canSignOff = Boolean(data?.run_id) && !saved;

  // Reset on every entity change.
  //
  // Curated examples used to ship a pre-baked, cited summary fetched from
  // `/curated-narratives/<lei>.json`, so a first-time visitor saw the feature
  // with no model call. **That is gone, deliberately.** Those files had to be
  // regenerated by hand whenever the prompt, the model or the curated set
  // changed, and a stale one is worse than none: it shows text generated by a
  // prompt version the product no longer runs, under a run id nothing can
  // reproduce, labelled as though it were current. Every entity now takes the
  // same path — the "Generate summary" button — so what a reader sees was
  // generated by the model and prompt running today. Do not reintroduce a
  // cached tier without solving the staleness first.
  useEffect(() => {
    setData(saved?.narrative ?? null);
    setError(null);
    setDisp(dispFromRecord(saved?.dispositions ?? null));
    setCommentOpen({});
    setSaveState("idle");
    setEvidenceExpanded(false);
    setReviewed(Boolean(saved?.dispositions?.reviewed));
  ...
  useEffect(() => {
    if (!dirtyRef.current || !data?.run_id || saved) return;
  ...
  }, [data, disp, reviewed, onExportPayload]);
    ...
    <section className="px-4 py-[18px] sm:px-6 sm:py-[22px] border-b border-oo-rule">
      ...
      {!data && !collapsed && saved && (
        <p className="mt-3 text-oo-small text-oo-muted">
          No summary was saved with this report.
        </p>
      )}
      ...
      {data && !collapsed && (
        <div className="mt-4">
          ...
          <div className="mt-3.5 flex items-center flex-wrap gap-x-4 gap-y-2 text-oo-small">
            {saved && (
              <span className={reviewed ? "text-oo-ok-text font-medium" : "text-oo-muted"}>
                {reviewed ? "Marked as reviewed when saved" : "Not marked as reviewed when saved"}
              </span>
            )}
            {canSignOff && (
              <button
                ...
                {(evidenceCollapsed
                  ...
                  return (
                    <li key={c.id} className="text-oo-small text-oo-ink">
                      ...
                      {saved && <SavedDisposition value={disp[c.id]} />}
                      {canSignOff && (
                        <DispositionControls
                          claimId={c.id}
                          value={disp[c.id]}
