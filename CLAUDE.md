...
**Estonian adapter**: `ariregister.py` is now a public web scraper — `GET ariregister.rik.ee/eng/company/{reg}/company_print_json`. No credentials. The previous SOAP/X-Road approach (Phase 37) used `ariregxmlv6.rik.ee` with `ARIREGISTER_USERNAME`/`ARIREGISTER_PASSWORD` credentials from a paid RIK contract that turned out not to grant data access. Do NOT revert to SOAP. The HTML parser extracts officers (→ Estonian role codes) and shareholders (person vs entity from ID code length). **BOs: none since 2026-07-10** — Estonia withdrew anonymous BO access (an auth prompt replaced the table); `_parse_beneficial_owners` is retained and degrades to `[]` (issues #22/#28). `map_ariregister()` in mapper.py is unchanged.
...
- **BO history REMOVED 2026-07-10** (issue #22): `_bo_events()` and the `tegelikudKasusaajad_v2` call are gone — Estonia withdrew open-data BO access (AMLD6 transposition; the replacement Beneficial Owners API is entitlement-gated, not for anonymous open-data tools). Do not reinstate. Registry-card history (`detailandmed_v2` — names, addresses, forms, statuses, board, shareholders) is unaffected.
